...
class SegmentAnythingUI:
    ...
    def generate_mask(self, display=True):
        ...
        # input_point = np.array([self.femur_point])

        for i, bone in enumerate(self.bones):
            # bone = "femur"
            input_label = self.label_map[bone]
            # print(f"input point: {input_point}, bone: {bone} and label: {input_label}")
            # print(f"input shapes: {input_point.shape} \n {input_label.shape}")
...
#         # self.bones = ["femur", "tibia"]
#         self.bones = ["femur"]
#         self.label_map = {"femur": np.array([1, 0])}
#         # self.label_map = {"femur": np.array([1, 0]), "tibia": np.array([0, 1])}
#         self.mask_colors = np.array([[30, 144, 255, 153], [255, 144, 30, 153]])
...
#         # print(f"\n input point is: {input_point}")
#         # for i, bone in enumerate(self.bones):
#         # print(f"i: {i} and bone: {bone}")
#         # input_label = self.label_map[bone]
...
#         mask = cv2.erode(mask.astype(np.uint8), kernel, iterations=1)
#         mask = cv2.dilate(mask, kernel, iterations=1).astype(bool)
#         print(f"self. mask shape: {mask.shape}")
#         self.masks.append(mask)
#         print(f"added to self.masks: {len(self.masks)}")
#         self.compute_mask_points()
...
#         while True:
#             key = cv2.waitKey(1) & 0xFF
#             print(f"self.mask_generated: {self.mask_generated}")
#             if key == 27:  # Esc key
#                 self.reset()
#             if self.femur_point is not None:
#                 # self.update_prompt("Click a point on the Bone")
...
#             # Update the prompt after the femur is selected
#             # if self.femur_point is not None and self.tibia_point is None:
#             #     # self.update_prompt("Click a point on the Tibia")s
...
#         result = (self.masks, [self.femur_point], self.mask_points)
#         self.reset()
#         return result
